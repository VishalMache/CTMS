...
const submitTest = async (req, res) => {
    try {
        ...
        if (!parsed.success) {
            console.error('Submit Validation Error:', parsed.error.flatten());
            return res.status(400).json({
                success: false,
                message: 'Invalid payload: ' + JSON.stringify(parsed.error.flatten().fieldErrors)
            });
        }
        ...
        if (!studentRecord) return res.status(404).json({ success: false, message: 'Student profile not found. Please complete your profile first.' });
        ...
        console.log(`[MOCK TEST] Student ${studentId} submitted test ${testId}: ${score}/${test.totalMarks}`);
        res.json({ success: true, result });
    } catch (error) {
        console.error('Submit Error:', error);
        res.status(500).json({ success: false, message: error.message || 'Server error during submission' });
    }
};
...
// ────────────────────────────────────────────────────────────
// GET /api/mock-tests/leaderboard  (Any authenticated user)
// Overall leaderboard combining all test results
// ────────────────────────────────────────────────────────────
    ...
    try {
        const results = await prisma.mockTestResult.findMany({
            include: {
                ...
                test: {
                    select: { title: true, type: true, totalMarks: true }
                }
            },
            orderBy: { score: 'desc' },
            take: 50
